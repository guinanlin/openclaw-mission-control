...
async def create_gateway(
    ...
    await service.assert_gateway_runtime_compatible(
        url=payload.url, token=payload.token, password=payload.password
    )
...
async def update_gateway(
    ...
    if "url" in updates or "token" in updates or "password" in updates:
        ...
        next_password = updates.get("password", gateway.password)
        if next_url:
            await service.assert_gateway_runtime_compatible(
                url=next_url, token=next_token, password=next_password
            )
